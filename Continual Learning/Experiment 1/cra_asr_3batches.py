# -*- coding: utf-8 -*-
"""CRA_ASR_3batches.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1FrJaZqWhyp5B1AJv_kGxEhKgvrS3FxCE

C3 batches- used for CRA analysis, accuracy vs lambda for diff batches
"""
...
def cra(old_model: nn.Module, model:nn.Module,dataset:torch.utils.data.DataLoader, old_labels):
  loss = 0
  criterion = torch.nn.CrossEntropyLoss()
  inp = dataset
  #inp = Variable(dataset).cuda()
  #print(inp.shape)
  old_pred = old_model.final_layer(model.speech_model(inp))
  #print(x.shape)

  l = criterion(old_pred, old_labels)
  #print(l)
  loss+=l.item()
...
for e in range(N_EPOCHS):
  ...
  for i, batch in enumerate(tqdm_notebook(train_dlB)):
    ...
    """
    cra_loss = 0
    cra_loss = cra(net.speech_model, new_net.speech_model,old_batch[0], old_labels[1])
    # Compute the loss
    loss = criterion(y, labels) + lamda*cra_loss
...
for e in range(N_EPOCHS):
  ...
  print("Training accuracy:", running_accuracy/float(len(train_setC)),
        "Training loss:", running_loss/float(len(train_setC)), "CRA Loss:", lwf_loss)
