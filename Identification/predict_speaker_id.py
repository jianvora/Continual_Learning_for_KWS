...
def create_batches_rnd(batch_size,data_folder,wav_lst,N_snt,wlen,lab_dict,fact_amp):
 ...
 for i in range(batch_size):
  ...
  # print(wav_lst[snt_id_arr[i]].upper())
 ...
 inp=Variable(torch.from_numpy(sig_batch).float().cpu().contiguous())
 lab=Variable(torch.from_numpy(lab_batch).float().cpu().contiguous())
...
path_to_eval = "P0002/enroll/"
num_files = (len([name for name in os.listdir(path_to_eval)]))

sig_batch=np.zeros([num_files,wlen])
 # 
rand_amp_arr = np.random.uniform(1.0-0.2,1+0.2,num_files)
count = 0

for i in os.listdir(path_to_eval):

  [signal, fs] = sf.read(path_to_eval+i)
  ...
  sig_batch[count,:]=signal[int(snt_beg):int(snt_end)]*rand_amp_arr[count]
  count += 1
...
#      lab= Variable((torch.zeros(N_fr+1)+lab_batch).cpu().contiguous().long())
#      pout=Variable(torch.zeros(N_fr+1,class_lay[-1]).float().cpu().contiguous())
#      count_fr=0
#      count_fr_tot=0
